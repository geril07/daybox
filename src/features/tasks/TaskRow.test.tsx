...
beforeEach(() => {
  useAppStore.setState({
    ...
    settings: {
      timer: { focusDuration: 25, shortBreakDuration: 5, longBreakDuration: 15, longBreakInterval: 4, autoStartBreaks: false, autoStartPomodoros: false, alarmSound: 'bell', alarmVolume: 0.5, alarmRepeat: 3 },
      theme: 'light', weekStartDay: 1,
    },
    view: 'today', browseDate: null, focusedTaskId: null, stickyGroupId: null,
  })
})
...
describe('TaskRow', () => {
  ...
  it('toggles completion on checkbox click', () => {
    ...
    const checkbox = document.querySelector('.rounded-full.border') as HTMLButtonElement
  ...
  it('focuses task on focus button click', () => {
    ...
    useAppStore.setState({ tasks: [task] })
    render(<TaskRow task={task} />)
    const focusBtn = document.querySelector('[title="Focus"]') as HTMLButtonElement
