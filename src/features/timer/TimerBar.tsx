...
export default function TimerBar() {
  ...
  useEffect(() => {
    if (remainingMs <= 0 && isRunning) {
      if (!alarmPlayedRef.current) {
        ...
        sendNotification(
          phase === 'focus' ? 'Focus complete!' : `${phase === 'shortBreak' ? 'Short break' : 'Long break'} complete!`,
          focusedTask ? `Task: ${focusedTask.title}` : undefined,
        )

        if (phase === 'focus' && focusedTask) {
          updateTask(focusedTask.id, { pomoCompleted: focusedTask.pomoCompleted + 1 })
        }

        const nextPhase = getNextPhase(phase, sessionPomoCount, settings.longBreakInterval)
        ...
        const newSessionCount =
          nextPhase === 'longBreak' || (nextPhase === 'shortBreak' && phase === 'focus')
            ? sessionPomoCount + 1
            : sessionPomoCount
  ...
  }, [remainingMs, isRunning, phase, focusedTask, sessionPomoCount, settings, updateTask])
  ...
  const phaseColor = phase === 'focus' ? 'var(--accent)' : phase === 'shortBreak' ? 'var(--break-color)' : 'var(--lbreak-color)'
  ...
  return (
    <div style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border)' }}>
      <div className="h-[2px] overflow-hidden" style={{ background: 'var(--border)' }}>
        <div
          className="h-full transition-[width] duration-900 linear rounded-r-[2px]"
      ...
      <div className="flex items-center gap-3.5 px-7 py-2.5 max-w-[680px] mx-auto">
        <div className="flex-1 min-w-0 flex flex-col gap-[1px]">
          <span className="text-[10.5px] font-semibold uppercase tracking-[0.8px]" style={{ color: phaseColor }}>
            {phaseLabel}
          </span>
          <span className="text-[12.5px] truncate" style={{ color: focusedTask ? 'var(--fg-2)' : 'var(--fg-3)' }}>
            {focusedTask ? focusedTask.title : 'No task focused'}
          </span>
        </div>
        <span
          className="text-[30px] font-medium tracking-[1px] shrink-0 min-w-[80px] text-center transition-colors duration-300"
          style={{
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            className="w-[40px] h-[40px] rounded-full flex items-center justify-center transition-all duration-140"
            style={{
        ...
        <div className="flex gap-[3px] items-center shrink-0 min-w-[40px]">
          {sessionDots.map(i => (
            <span
              key={i}
