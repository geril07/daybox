...
The system SHALL provide domain-agnostic validation helpers in `src/shared/utils/save-helpers.ts` that eliminate repeated patterns across feature save slices. The helpers SHALL NOT import from `src/modules/*` or reference feature-owned types.
...
### Requirement: SaveSliceMap interface enables typed postPrepare parameter

The system SHALL declare a `SaveSliceMap` interface in `src/shared/save-slice/map.ts` with an empty body. The `SaveSlice` contract's `postPrepare` callback SHALL accept `allSlices: SaveSliceMap` as its second parameter by default. The data-portability registry SHALL augment `SaveSliceMap` via TypeScript's `declare module` directive with the concrete slice type for each registered slice name. This provides compile-time type safety for `allSlices` lookups in `postPrepare` implementations with zero runtime cost and no import cycles.

#### Scenario: Registry augments SaveSliceMap for each registered slice

- **WHEN** the data-portability registry is compiled
- **THEN** it contains a `declare module` block that adds each registered slice's name as a property of `SaveSliceMap` with the slice's `TCurrent` type
- **AND** the augmentation does not emit any runtime code

#### Scenario: postPrepare receives typed allSlices

- **WHEN** a slice's `postPrepare` callback accesses `allSlices.groups`
- **THEN** TypeScript resolves the type to the groups slice's `TCurrent` (e.g., `GroupsSaveSliceCurrent`)
- **AND** `allSlices.groups.groups.map(...)` has full IntelliSense
...
**Reason**: Cross-slice invariant repair now lives in each dependent slice's `postPrepare` hook. The centralized `normalizeCrossSliceInvariants` function is deleted. Slice-local repairs (default-group restoration, duplicate-id detection) remain in `prepareImport` as before.

**Migration**: Any cross-slice repair logic currently in `normalizeCrossSliceInvariants` moves to the dependent slice's `postPrepare`. For the existing dangling-group-reference repair, the tasks slice gains a `postPrepare` implementation. The `src/modules/data-portability/normalize.ts` file is deleted and its export removed from the data-portability barrel.
