...
- **Phase identity is misplaced.** The current phase (focus / short break / long break) is signaled only by scattered color cues and a tiny label that sits _on top of the focused task name_ (`TimerBar.tsx:213-219`). The 30px digits — the most prominent element — carry no phase identity at all. Phase is the _timer's_ mode; bolting its label onto the _task_ conflates two different concepts.
