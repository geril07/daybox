...
- **Task creation no longer depends on keydown-only Enter handling** — `AddTaskRow` submits through a native form and exposes a coarse-pointer-only submit button, so mobile users can create tasks from either the soft-keyboard submit action or an explicit tap target.
...
- `src/features/tasks/components/AddTaskRow.tsx` — wrap the add-task input row in a native form, keep Enter interception only for highlighted group suggestions, and add a coarse-pointer-only submit button.
