...
The system SHALL allow the user to initiate a Google OAuth flow from the settings drawer. The flow SHALL use the Authorization Code + PKCE flow mediated by the stateless backend at `/api/auth/*` (owned by the `oauth-backend` capability), SHALL request the `drive.file`, `openid`, and `email` scopes, and SHALL result in a refresh token stored encrypted in an `HttpOnly` cookie set by the backend. The SPA SHALL NOT handle the authorization code, the client secret, or the refresh token at any point. The connect action SHALL perform a full browser redirect to `/api/auth/start`; after the OAuth callback completes, the backend SHALL redirect back to `/?connected=1` and the app shell SHALL hydrate the connection state from `/api/auth/status`. The OAuth client id and secret SHALL be read from the `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` server-only env vars; the `VITE_GOOGLE_CLIENT_ID` build-time env var SHALL NOT be used.
...
- **THEN** the browser is redirected to `/api/auth/start`
- **AND** the backend redirects to Google's consent screen with the PKCE challenge and `access_type=offline`
- **AND** after the user grants permission, the backend exchanges the code, stores the encrypted refresh-token cookie, and redirects back to `/?connected=1`
- **AND** the app shell hydrates the connection state from `/api/auth/status` and the panel re-renders in the connected state showing the account email and the Back up / Restore / Disconnect controls

#### Scenario: User denies the consent prompt

- **WHEN** the user closes the Google consent screen without granting permission
- **THEN** Google redirects back to the callback without a code
- **OR** the user returns to the app without completing consent
...
#### Scenario: The server env vars are missing

- **WHEN** `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, or `TOKEN_ENC_KEY` is not set on the server
- **THEN** the `/api/auth/start` endpoint responds with a 500
- **AND** the panel shows an inline "Google Drive is not configured for this build" message
...
The system SHALL allow the user to disconnect at any time from the connected state. Disconnecting SHALL call `POST /api/auth/disconnect` (which clears the `__Host-db_rt` cookie and revokes the refresh token at Google) and SHALL clear the local `dayboxFileId`, `backupFileSpace`, and `lastBackupAt` from the persisted `daybox-google-drive` state, returning the panel to the disconnected state. Disconnecting SHALL NOT delete the file on Drive — the user's data on Drive is theirs to keep.

#### Scenario: User disconnects

- **WHEN** the user clicks "Disconnect" in the connected panel
- **THEN** the SPA calls `POST /api/auth/disconnect`
- **AND** the backend clears the `__Host-db_rt` cookie and attempts to revoke the refresh token at Google
- **AND** the `daybox-google-drive` persisted state is cleared
...
#### Scenario: Backup fails on token expiry and silent refresh fails

- **WHEN** the cached access token has expired or is missing
- **AND** the call to `/api/auth/refresh` returns 401 (refresh token revoked or cookie lost)
- **THEN** the panel shows the existing "reconnect" inline message (the `token-expired` error kind)
- **AND** the user can click Connect to re-authorize via the server OAuth flow
- **AND** `lastBackupAt` is NOT updated
- **AND** `dayboxFileId` is NOT changed
- **AND** the remembered `dayboxFileId` metadata is preserved so the reconnect reuses the existing backup file

### Requirement: User can restore manually from Google Drive

The system SHALL allow a connected user to restore canonical save snapshot data from Google Drive on demand, including on a browser or device that has not performed a backup locally. The restore SHALL require explicit user confirmation before downloading the file, preserving the current cancellation behavior. After confirmation, restore SHALL acquire a fresh access token from `/api/auth/refresh`, download the visible Drive file identified by `dayboxFileId` when available. If `dayboxFileId` is missing, restore SHALL list the user's My Drive root and find an accessible non-trashed `daybox.json` by name, then store the discovered file id as the current visible root backup id. The downloaded content SHALL be passed through `prepareSnapshotImport` from `@/modules/data-portability`, and the prepared snapshot SHALL be committed through `commitSnapshotImport` only when preparation succeeds.
...
- **THEN** the store obtains a fresh access token from `/api/auth/refresh`
- **AND** the file is downloaded from Drive by id
...
#### Scenario: Restore fails on token expiry and silent refresh fails

- **WHEN** the call to `/api/auth/refresh` returns 401 during a restore
- **THEN** the panel shows the existing "reconnect" inline message (the `token-expired` error kind)
- **AND** no download is performed
- **AND** no local state is modified
- **AND** the remembered `dayboxFileId` metadata is preserved

### Requirement: Panel displays connection state, account email, and last backup time

The system SHALL show in the Google Drive section of the settings drawer: the connection state (disconnected / connected) hydrated from `/api/auth/status` on app boot, the account email when connected, that backups are stored as a visible `daybox.json` file in the user's Google Drive root, that Restore can search Google Drive root for `daybox.json`, and the `lastBackupAt` timestamp as a human-readable relative duration (e.g. "2 days ago") when a backup has been performed. These displays are informational only and do not nag or prompt the user to act.

#### Scenario: Disconnected panel

- **WHEN** `/api/auth/status` reports `{ connected: false, email: null }`
...
- **WHEN** the user is connected (per `/api/auth/status`) but this browser has never backed up
...
#### Scenario: Reconnect message on expired refresh token

- **WHEN** `/api/auth/refresh` returns 401 during a backup or restore
- **THEN** the connected panel shows the existing "reconnect" inline message (the `token-expired` error kind)
- **AND** the Connect button is presented so the user can re-authorize via the server OAuth flow
- **AND** the remembered `dayboxFileId` and `lastBackupAt` metadata remain visible and intact
