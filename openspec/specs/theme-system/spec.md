...
Define the theme mode (light / dark / system) and preset (named color palette) model, the JS-driven token application engine, preset registry with fallback rules, migration from the legacy binary format, and the `useTheme` hook API.
